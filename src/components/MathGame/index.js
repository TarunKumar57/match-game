...
class MathGame extends Component {
  ...
  onReset = () => {
    ...
    this.setState({score: 0, timeInSeconds: 60, isReStart: true})
  }
  ...
  onThumbnail = id => {
    ...
    if (matchImg.id === id) {
      this.setState(prevState => ({
        matchImg: imagesList[Math.ceil(Math.random() * imagesList.length)],
  ...
  renderHeader = () => {
    const {score, timeInSeconds} = this.state
    return (
      <nav>
        <img
        ...
        <ul className="score-timer-container">
          <li>
            <p className="score-text">
              Score:<span className="timer-score-text">{score}</span>
            </p>
          </li>
          <li className="timer-container">
            <img
              ...
              className="timer-img"
            />
            <p className="timer-score-text">{timeInSeconds} sec</p>
          </li>
        </ul>
      </nav>
    )
  ...
  renderMatchGame = () => {
    const {activeTabId, thumbnailList, matchImg} = this.state
    return (
      <div className="list-container">
        <ul className="match-list">
          <MathMatch key={matchImg.id} matchDetails={matchImg} />
        </ul>
  ...
  renderScoreCard = () => {
    ...
    return (
      <div className="score-card-container">
        ...
        <button type="button" className="play-again-btn" onClick={this.onReset}>
          <img
  ...
  render() {
    const {timeInSeconds, isReStart} = this.state
    return (
      <div>
        {this.renderHeader()}
        <div className="container">
          {timeInSeconds !== 0 && isReStart
